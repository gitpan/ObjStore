...
The easiest way to evolve data is to rebuild it from scratch.
Therefore, the first thing to consider is how to partition your data
to avoid evolution altogether.  It is entirely unnecessary to store
all data in a single database.  Databases interoperate and be used
simultaniously.  In fact, the primary reason to keep data in the same
database is to be able to use real pointers to connect objects
together.  By splitting your data into logical chunks, you should be
able to avoid evolution in most cases.
...
Part of update transaction commit time is proportional to the number
of pages with writes, not the amount of data written.
...
C<ObjStore::Notification>).  The two technologies address different
...
Perl now integrates with C<Java> and the C<SwingSet / AWT> API.  (Of
course, this is a moot point if you can use C<Qt> - http://www.troll.no/.)
