...
[This bug entry might have been entirely fixed!]

Perl signal handlers are not bulletproof.  Since ObjectStore makes
aggressive use of signals, pathelogical failure is not impossible.
Currently, it is recommmended that you avoid lock timeouts and
deadlocks altogether.  This can be accomplished by restricting each
database to a single writer, and using mvcc mode for readers.  (This
is a good architecture anyway!)

If you cannot use this architecture, please test the reliability of
your signal handlers before making a large code investment.  This is
worth testing because with conservative and careful use of perl signal
handlers, you should be able to avoid triggering any of the known
failure modes.
...
[A proper fix is on the TODO list.]
