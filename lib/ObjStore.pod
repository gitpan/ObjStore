...
and provides little benefit beyond our read-only scheme.)  For
example,
...
name.  You cannot index direct keys more than once.  There is no such
restriction on indirect keys.
...
Without the 'now' parameter, notification will take place after
commit.
...
extended by the I<bless from> and the I<bless to> classes.  (This is
...
It is critical that the copy is avoided, since keys can be relocated
when arrays need to grow.  OSSVPV pointers are never relocated.
...
been discussed on the perl-porters mailing list.
...
directory, I<and C<SQL> (never again!)>
