...
and the a shiver runs through hair on the back of your neck.  I hope
you're a quick learner!  Many people haven't made it!

Learning something new is hard work.  This may be true, but isn't it
easier to get ahead than be left behind?  It's a preditory world.
...
your database and walk around from the inside.  You can also invoke
methods on your objects or write custom reports (in perl, of course
:-).  (Similar to C<isql>/C<wisql>.)

=item * osperlserver

Provides a server framework including remote method invokation, job
scheduling, and server-object collaboration services.  Completely
generic.
...
database or segment) in place of C<$near> and the new object will be
...
in doubt (see C<ObjStore::Ref::POSH_ENTER>).  Currently, there are two
...
A C<SEGV> (if you're lucky) or garbled data can result.
...
All the index representations supplied by default avoid copying
records' keys.  The keys are used in-place and may be marked
read-only.  This is an advantage until the data-set cannot fit in the
client-side cache.  Once you data gets very large, an alternate
strategy is needed.  Consider enhancing the current representations or
designing a new one.
...
If you need something that is not supported by default, it is likely
that the wanted behavior can be implemented with new representations.
(Virtual sunglasses not included. :-)

=head2 And Access Paths (Oh My!)

If you cross your eyes, you may be able to visualize an array of
references or cursors as an access path.  Two simple implementations
are included:
...
While there does not exist a central formalized schema for a perl
database the C<ospeek> utility outputs a sample of data content and
structure.  C<ospeek> never outputs more than a brief summary no
matter how much stuff is in your database.
...
they were conceived, why is it still common practice to create
...
compatibility.  (Database are just a rich API.)
...
your data to improve your ability to evolve.
...
compatibility.  It reduces the scope of the problem in comparison to
...
C<ObjStore::Index> and L<ObjStore::Table3>.
...
mostly mirrors the C<ObjectStore> C<C++> API.  Refer to the
C<ObjectStore> documentation for exact symantics.
...
errors, just in case exceptions are not automatically re-thrown.  I am
already regretting that I added this mode to begin with.
...
Built-in automatic deadlock retry is not supported.  Support was
withdrawn not because it doesn't work but because it doesn't make much
sense in perl.  Deadlock retry is so easy to implement yourself, you
should (if you even need it).
...
flexiblity potential is very great.  It might even be too flexible!
