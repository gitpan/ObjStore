...
the ObjectStore documentation for exact symantics.
...
This is the recommended API.  If the given root is not found, creates
a new one.  Sets the root's value if $new_value is defined.  Returns
...
interface.  (See perltie(3) or http://www.perl.com more info.)
...
hashes assume an expected cardinality of around ten.  They are created
in the same segment as the enclosing hash.
...
While there is no official schema for a database, the C<ospeek>
...
=head1 WHY IS PERL BETTER FOR DATABASES THAN C++, JAVA, OR SQL?
...
You don't have to understand anything about the persistent
representation.  Just know that:
...
=head2 NEED MORE SPEED?

If you need more speed, wait 1-4 months for Perl5 to get a compiler
and kernel threads.  Or keep reading and learn about dynamically
linking your own custom C++ objects.
...
requirements than transient values.  The dress code is upscale, New
York style attire.
...
The C<'_at'> method can be used to access scalars as
