...
ObjStore - Perl extension for ObjectStore ODBMS
...
C<ObjStore::UNIVERSAL> base class is used only for objects or
collections.  The C<'_at'> method can be used to access scalars as
C<'ObjStore::Raw'> objects in some cases, but there are no methods to
manipulate them except from C++.  Currently, only C<bless> makes use
of the C<_at> method.
