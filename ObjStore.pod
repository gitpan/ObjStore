...
$class($where, ...)'>.  You may pass any persistent object
(or database, or segment) in place of $where and the new object will
...
First-class cursors are incomplete and under construction.  If you can
avoid them, do so!
...
will feel right at home.  See C<ObjStore::Table2>.
...
See C<ObjStore::HV::Database>, C<ObjStore::Table2::Database>
...
slots:
  ...
  new ObjStore::HV($where, { key => 'value' });  # 1 slot
  new ObjStore::AV($where, [1..3]);              # 3 slots

Be aware, however, that any transient structures you pass through the
stargate are dismembered as they are copied to insure that they can be
transiently garbage collected.  [MAYCHANGE]
...
=item * INTEGRATION WITH STANDARD PERL EXCEPTIONS

Once there I<are> standard perl exceptions.

=item * LEANER COLLECTIONS

The C<ObjectStore> collections are weighted down with embedded index and
query support.  Worse, C<ObjectStore> cursors cannot use references, only
...
is" without express or implied warranty.  Perl / C<ObjectStore> is
...
C<ObjStore::Tutorial>, C<ObjStore::Table2>, examples in the t/
