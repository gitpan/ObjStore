...
It is possible to use C<eval> within transactions, but B<you must not>
use the C<ObjectStore> API or access persistent memory.  This is due
to the excellent but imperfect integration of C<ObjectStore>
exceptions and perl exceptions.  In general, you should just use
C<begin> instead of C<eval>.  (Also see C<ObjStore::Process> and
C<ObjStore::Posh::Cursor>.)
...
adopted.  Therefore, I would like to express a poignant "thank you" to
all the wonderful teachers with whom I've had the opportunity of
...
belief to be true.  In short, if you have never studied with a master,
I highly recommend it!  (It is a priceless opportunity!)  If you look
sincerely, you will be able to find one.  They are waiting for you...!
:-)
